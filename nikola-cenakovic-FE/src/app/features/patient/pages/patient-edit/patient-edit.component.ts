...
export class PatientEditComponent implements OnInit {
  ...
  loadPatient(){
    ...
    this.httpPatientService.findById(id).subscribe(pat =>{
      ...
      this.loadPractitionerList(pat.organization.idOrganization);
    });
  }
  loadPractitionerList(id:number){
    this.httpPractitionerService.findAllDoctorOfMadicine().subscribe(prcList =>{
      this.practitionerList = prcList;
      this.practitionerListFromOrganization = this.practitionerList?.filter(item=>item.organization.idOrganization===id);
    });
  }
  ...
  editPatient(){
    ...
    this.httpPatientService.edit(patient)
    .subscribe(()=>{
      this.router.navigate(['patient']);
      this.toastService.showToast("Patient have been edited.",
            { className: 'bg-success text-light' });
    },err=>{
      this.toastService.showToast("Invalid data patient not be edited!",
